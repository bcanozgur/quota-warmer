...
- **`CredentialStore`** — reads tokens from Keychain, env, and on-disk JSON. Claude: a mirror item QuotaWarmer owns, then Keychain service `Claude Code-credentials` (+ `CLAUDE_CONFIG_DIR`-hashed variants) **read through `/usr/bin/security`**, then `CLAUDE_CODE_OAUTH_TOKEN`, then `~/.claude/.credentials.json`. The subprocess read is not an accident: Claude Code writes its item with the `security` tool, so the item's *partition list* is `apple-tool:` while QuotaWarmer's is `teamid:…`. A partition mismatch makes macOS demand the login password on every read, and clicking "Always Allow" only appends a trusted app — it never adds the partition, which is why the dialog used to return on every ~8h token rotation. Reading through the already-trusted tool is silent. Never replace it with a plain in-process `SecItemCopyMatching`. Codex: `$CODEX_HOME`/`~/.config/codex`/`~/.codex` `auth.json` then Keychain. Lookups tolerate many key spellings and dotted paths.
...
- **No Keychain dialog from a background poll**: every read that a timer can reach runs inside `withoutKeychainDialogs` (`SecKeychainSetUserInteractionAllowed(false)`). `LAContext.interactionNotAllowed`/`kSecUseAuthenticationContext` looks like it does this but governs only the data-protection keychain — it left the login-keychain dialog fully enabled. Only a user-initiated Refresh may prompt.
